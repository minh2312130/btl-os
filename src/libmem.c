...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  pthread_mutex_lock(&mmvm_lock); // them
  ...
  /*
  */
 caller->mm->symrgtbl[rgid].rg_start = old_sbrk;        // bắt đầu từ limit cũ
 caller->mm->symrgtbl[rgid].rg_end = old_sbrk + size; // size chưa aligned
...
int liballoc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  int addr;
  int can_alloc = __alloc(proc, 0, reg_index, size, &addr);
  if (can_alloc != 0)
  {
...
int libfree(struct pcb_t *proc, uint32_t reg_index)
{
  ...
  int can_free = __free(proc, 0, reg_index);
  if (can_free != 0)
  {
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    */
    ...
    regs.flags =0;
    /* Update page table */
    uint32_t swaptype = caller->active_mswp_id;
    pte_set_swap(&vicpte,swaptype , swpfpn);
...
int libwrite(
    ...
    uint32_t offset)
{
  int can_write = __write(proc, 0, destination, offset, data);
#ifdef IODUMP
...
int find_victim_page(struct mm_struct *mm, int *retpgn)
{
  ...
  // find the first page in the list ( the last node)
...
int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
{
  ...
  struct vm_rg_struct *rgit_prev = cur_vma->vm_freerg_list;
  if (rgit == NULL)
    return -1;
  ...
  while(rgit != NULL){
    int check = rgit->rg_end - rgit->rg_start;
